...
store | The session store instance, defaults to a new MemoryStore instance. | The default is an empty string, which uses a new MemoryStore instance. To use MongoDB as the session store, specify a MongoDB connection string: `"mongodb://username:password@host/databaseName"`. To use Redis, specify a Redis server's IP address and port number: `"127.0.0.1:6379"`.
cookie  | |
